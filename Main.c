...
// Corss platform clear screen and pause functions
#ifdef _WIN32
#include <Windows.h>
...
void loadLeagueData(struct Team teams[4], int *gameCount)
{
    ...
    if (fp != NULL)
    {
        for (int i = 0; i < 4; i++)
        {
            ...
            if (fscanf(fp, "%s %d %d", teams[i].teamName, &teams[i].wins, &teams[i].losses) != 3)
                break;
    ...
    if (confirm == 'Y' || confirm == 'y')
    {
        *gameCount = 0;
        ...
        int jerseyNumbers[4][5] = {
            {4, 7, 10, 11, 14}, {4, 5, 6, 10, 9}, {4, 7, 13, 6, 5}, {4, 5, 6, 7, 8}};
...
void averages(struct Team teams[4])
{
    ...
    { // SORT BY POINTS
        for (int i = 0; i < 4 - 1; i++)
        {
            for (int j = 0; j < 4 - i - 1; j++)
            {
                float avg1 = (sortedTeams[j].wins + sortedTeams[j].losses > 0) ? (float)(sortedTeams[j].MainPlayers[0].totalPoints + sortedTeams[j].MainPlayers[1].totalPoints + sortedTeams[j].MainPlayers[2].totalPoints + sortedTeams[j].MainPlayers[3].totalPoints + sortedTeams[j].MainPlayers[4].totalPoints) / (sortedTeams[j].wins + sortedTeams[j].losses) : 0;
                float avg2 = (sortedTeams[j + 1].wins + sortedTeams[j + 1].losses > 0) ? (float)(sortedTeams[j + 1].MainPlayers[0].totalPoints + sortedTeams[j + 1].MainPlayers[1].totalPoints + sortedTeams[j + 1].MainPlayers[2].totalPoints + sortedTeams[j + 1].MainPlayers[3].totalPoints + sortedTeams[j + 1].MainPlayers[4].totalPoints) / (sortedTeams[j + 1].wins + sortedTeams[j + 1].losses) : 0;
    ...
    { // SORT BY REBOUNDS
        for (int i = 0; i < 4 - 1; i++)
        {
            for (int j = 0; j < 4 - i - 1; j++)
            {
                float avg1 = (sortedTeams[j].wins + sortedTeams[j].losses > 0) ? (float)(sortedTeams[j].MainPlayers[0].totalRebounds + sortedTeams[j].MainPlayers[1].totalRebounds + sortedTeams[j].MainPlayers[2].totalRebounds + sortedTeams[j].MainPlayers[3].totalRebounds + sortedTeams[j].MainPlayers[4].totalRebounds) / (sortedTeams[j].wins + sortedTeams[j].losses) : 0;
                float avg2 = (sortedTeams[j + 1].wins + sortedTeams[j + 1].losses > 0) ? (float)(sortedTeams[j + 1].MainPlayers[0].totalRebounds + sortedTeams[j + 1].MainPlayers[1].totalRebounds + sortedTeams[j + 1].MainPlayers[2].totalRebounds + sortedTeams[j + 1].MainPlayers[3].totalRebounds + sortedTeams[j + 1].MainPlayers[4].totalRebounds) / (sortedTeams[j + 1].wins + sortedTeams[j + 1].losses) : 0;
    ...
    { // SORT BY ASSISTS
        for (int i = 0; i < 4 - 1; i++)
        {
            for (int j = 0; j < 4 - i - 1; j++)
            {
                float avg1 = (sortedTeams[j].wins + sortedTeams[j].losses > 0) ? (float)(sortedTeams[j].MainPlayers[0].totalAssists + sortedTeams[j].MainPlayers[1].totalAssists + sortedTeams[j].MainPlayers[2].totalAssists + sortedTeams[j].MainPlayers[3].totalAssists + sortedTeams[j].MainPlayers[4].totalAssists) / (sortedTeams[j].wins + sortedTeams[j].losses) : 0;
                float avg2 = (sortedTeams[j + 1].wins + sortedTeams[j + 1].losses > 0) ? (float)(sortedTeams[j + 1].MainPlayers[0].totalAssists + sortedTeams[j + 1].MainPlayers[1].totalAssists + sortedTeams[j + 1].MainPlayers[2].totalAssists + sortedTeams[j + 1].MainPlayers[3].totalAssists + sortedTeams[j + 1].MainPlayers[4].totalAssists) / (sortedTeams[j + 1].wins + sortedTeams[j + 1].losses) : 0;
...
void acePlayerData(struct Team teams[4])
{
    ...
    for (int i = 0; i < 20 - 1; i++)
    {
        for (int j = 0; j < 20 - i - 1; j++)
        {
            ...
            if (choice == 1)
            {
                if (((float)playerPtrs[j]->totalPoints / gp1) < ((float)playerPtrs[j + 1]->totalPoints / gp2))
                    swap = 1;
            }
            else if (choice == 2)
            {
                if (((float)playerPtrs[j]->totalRebounds / gp1) < ((float)playerPtrs[j + 1]->totalRebounds / gp2))
                    swap = 1;
            }
            else if (choice == 3)
            {
                if (((float)playerPtrs[j]->totalAssists / gp1) < ((float)playerPtrs[j + 1]->totalAssists / gp2))
                    swap = 1;
            }
            else if (choice == 4)
            {
                // FIXED: Alphabetical sort using strcmp on the Player lastName
                if (strcmp(playerPtrs[j]->lastName, playerPtrs[j + 1]->lastName) > 0)
                    swap = 1;
            }
    ...
    else
    {
        ...
        while (fscanf(fp, "%d %s %s %d %d", &id, hTeam, aTeam, &hScore, &aScore) == 5)
        {
            ...
            if (isMatch)
            {
                ...
                if (hScore > aScore)
                {
                    if (strcmp(hTeam, teams[team1Idx].teamName) == 0)
                        t1Wins++;
                    else
                        t2Wins++;
                }
                ...
                {
                    if (strcmp(aTeam, teams[team1Idx].teamName) == 0)
                        t1Wins++;
                    else
                        t2Wins++;
                }
